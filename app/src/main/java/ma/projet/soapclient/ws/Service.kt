...
class Service {
    ...
    fun getComptes(): List<Compte> {
        ...
        try {
            transport.call(NAMESPACE + METHOD_GET_COMPTES, envelope)
    ...
    fun createCompte(solde: Double, type: TypeCompte): Boolean {
        ...
        return try {
            transport.call(NAMESPACE + METHOD_CREATE_COMPTE, envelope)
            val response = envelope.bodyIn as? SoapObject
            // Le backend retourne un objet Compte, pas un booléen
            // Si la réponse contient un objet Compte, c'est que la création a réussi
            response != null
        } catch (e: Exception) {
            e.printStackTrace()
            false
    ...
    fun deleteCompte(id: Long): Boolean {
        ...
        return try {
            transport.call(NAMESPACE + METHOD_DELETE_COMPTE, envelope)
